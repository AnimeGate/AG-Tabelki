...
function sendNetworkEvent(request: NetworkRequest): void {
  if (window.debugAPI) {
    // Log to network category with request data as arg
    const statusEmoji =
...
export function installNetworkInterceptor(): void {
  ...
  if (
    (window as unknown as { __networkInterceptorInstalled?: boolean })
      .__networkInterceptorInstalled
  ) {
    return;
  }
  ...
  (
    window as unknown as { __networkInterceptorInstalled: boolean }
  ).__networkInterceptorInstalled = true;
