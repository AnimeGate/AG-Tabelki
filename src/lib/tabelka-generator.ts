 ...
 * Scale: 7 for 1080p (width <= 1920), 14 for 4K (width > 1920)
 */
export function generateLogo(config: LogoConfig): string {
  // Scale 7 for 1080p, 14 for 4K
  const scale = config.width > 1920 ? 14 : 7;

  // Position scales with the logo
  const posX = scale * 7;
  const posY = scale * 7;

  return `Dialogue: 0,0:00:00.00,9:59:59.99,AnimeGate,Logo,0,0,0,,{\\fscx${scale}\\fscy${scale}\\pos(${posX},${posY})\\an5\\shad0\\1a&H40&\\3a&HFF&\\p1}${LOGO_SVG}`;
}
